...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      const gmailState = new URLSearchParams(window.location.search).get("gmail");
      if (gmailState === "config") {
        setOauthNotice("Gmail sign-in is required, but Google OAuth is not configured for this deployment yet.");
      } else if (gmailState === "error") {
        setOauthNotice("Google sign-in could not be started. Please try again.");
      } else if (gmailState === "invalid_state") {
        setOauthNotice("The Google sign-in session expired. Please connect Gmail again.");
      }
      ...
      setConnected(status.connected && Boolean(accountEmail));
  ...
  return (
    <main className="shell">
      ...
      <section className="hero">
        <div>
          ...
          <p className="lede">
            {connected
              ...
              : "Gmail is required for this app. We request read-only access so the agent can find job alerts and recruiter opportunities without being able to send, delete, archive, or modify your email."}
          </p>
        </div>
        <div className="heroActions">
          {connected ? (
            ...
            <a className="button primary" href="/api/auth/google/start">Connect Gmail to continue</a>
          )}
        </div>
      </section>
      ...
      {!connected ? (
        <div className="workspace" style={{ gridTemplateColumns: "1fr", marginTop: 18 }}>
          <section className="panel resultsPanel">
            {oauthNotice && <div className="alert">{oauthNotice}</div>}
            <div className="emptyState">
              <div className="emptyIcon">✉</div>
              <h3>Gmail sign-in is required</h3>
              <p>
                Connect your Google account first. After sign-in, the app creates a separate career profile for that Gmail account and unlocks job scanning and match ranking.
              </p>
      ...
      <footer>
        Gmail is required and accessed read-only. Career profiles are separated by connected Gmail account on this device.
      </footer>
    </main>
  );
}
